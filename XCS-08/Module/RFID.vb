...
Module RFID
    Public Function ReadTagChkUpStream() As Boolean
        ...
        If Mid(RdInfo, 3, 1) = "P" Then
            ReadTagChkUpStream = True
        Else
            ReadTagChkUpStream = False
        End If
        Exit Function
...
Comm_Error:
    ...
    Public Function CRC(DATABUF) As String
        Dim CRCSetting
        Dim j, i
        Dim b
        b = DATABUF
...
READRETRY:
        ...
        Do While FrmMain.RFID_Comm.BytesToRead <> 0
            My.Application.DoEvents()
        Loop
        Timeout = 0
        Do
            'If ABORTFLAG Or ESTOPFLAG = True Then GoTo COMM_INTERRUPT
            Thread.Sleep(20)
            Timeout = Timeout + 1
            If Timeout > 30 Then GoTo READINBYTE
            My.Application.DoEvents()
        Loop Until FrmMain.RFID_Comm.BytesToRead >= 5 ' 7
...
READINBYTE:
        ...
        Loop Until n = BCount
        FrmMain.TextBox5.Text = Str1 'Display the string received
        'Exit Function
        t1 = InStr(1, Str1, ",")
        t2 = InStr(t1 + 1, Str1, ",")
        t3 = InStr(t2 + 1, Str1, ",")
...
WRITERETRY:
        ...
        Do
            Thread.Sleep(20)
            Timeout = Timeout + 1
            If Timeout > 100 Then GoTo CHECKINBYTE
            My.Application.DoEvents()
        Loop Until FrmMain.RFID_Comm.BytesToRead >= 5 ' 8
        'Exit Function
CHECKINBYTE:
...
WRITE_COMM_TIMEOUT:
    ...
    Public Function Bin2Hex(Bin As String) As String
        ...
        For i = 1 To 4
            ...
            End Select
        Next
        Bin2Hex = Data(1) & Data(2) & Data(3) & Data(4)
    End Function
    ...
    Public Function Hex2Bin(Hexdata As String) As String
        ...
        For i = 1 To Datalen
            ...
            End Select
        Next
        Hex2Bin = TempBin(4) + TempBin(3) + TempBin(2) + TempBin(1)
    End Function

    Public Function Hex2Dec(Data As String) As String
        Dim Dec As Integer
        Dim CalDec
        Dim Char_r
        Dim n As Integer
...
WRITERETRY:
        ...
        Next
        FrmMain.RFID_Comm.Write(SendString, 0, SendString.Length)
        Do While FrmMain.RFID_Comm.BytesToWrite <> 0
            My.Application.DoEvents()
        Loop
        Timeout = 0
        Do
            Thread.Sleep(20)
            Timeout = Timeout + 1
            If Timeout > 30 Then GoTo CHECKINBYTE
            My.Application.DoEvents()
        Loop Until FrmMain.RFID_Comm.BytesToRead >= 5 ' 8
        'Exit Function
CHECKINBYTE:
...
WRITE_COMM_TIMEOUT:
    ...
    Public Function RD_MULTI_RFID(AddrNos As String, WordLength As Long) As String
        Dim ii
        Dim jj
        Dim SendString(7) As Byte
        Dim rcstring(7) As Byte
